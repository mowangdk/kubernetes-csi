...
func copyFlagsFromConfigToGlobalVars() {
	...
	attacherRetryIntervalStart = &config.Configuration.AttacherConfiguration.RetryIntervalStart
...
func main() {
	...
	// Version is a process-level operation, not a controller startup. In
	// particular, an individual controller returning must not turn it into
	// an errgroup failure, and no controller selection is required.
	if *showVersion {
