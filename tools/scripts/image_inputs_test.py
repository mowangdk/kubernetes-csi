        ...
        for valid in (True, False):
            with self.subTest(valid=valid), tempfile.TemporaryDirectory() as directory:
                ...
                (root / "release-tools/prow.sh").write_text(
                    'printf "stub-sourced %s %s\\n" "$CSI_PROW_KIND_IMAGES" "$CSI_PROW_KIND_VERSION"\nmain() { :; }\n')
                uname = scripts / "uname"
                ...
                if valid:
                    ...
                    self.assertEqual(result.stdout.strip(), "stub-sourced " + selection["node"]["image"] + " " + selection["kind_version"])
                else:
